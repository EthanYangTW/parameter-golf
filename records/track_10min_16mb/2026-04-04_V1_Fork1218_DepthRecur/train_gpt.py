...
class Hyperparameters():
    ...
    # Depth recurrence: repeat these physical layers once more
    ...
    # Spectral TTT: adapt singular values at eval time
    spectral_ttt_enabled = bool(int(os.environ.get('SPECTRAL_TTT', '1')))
    spectral_ttt_lr = float(os.environ.get('SPECTRAL_TTT_LR', 0.001))
    spectral_ttt_epochs = int(os.environ.get('SPECTRAL_TTT_EPOCHS', 3))
    spectral_ttt_chunk_tokens = int(os.environ.get('SPECTRAL_TTT_CHUNK', 65536))
    spectral_ttt_layers = os.environ.get('SPECTRAL_TTT_LAYERS', 'last3')  # last2, last3, all
    spectral_ttt_wd = float(os.environ.get('SPECTRAL_TTT_WD', 0.0))
...
class GPT(nn.Module):
    def __init__(self, h: Hyperparameters):
        ...
        self.recur_layers = sorted(set(
            int(x) for x in h.recur_layers_str.split(',') if x.strip()
        )) if h.recur_layers_str.strip() else []
        self._recurrence_active = False
        # Virtual layer mapping: physical layers + repeated layers inserted after the last recur layer
        if self.recur_layers:
            cutoff = max(self.recur_layers) + 1
            self._v2p_recur = list(range(cutoff)) + self.recur_layers + list(range(cutoff, h.num_layers))
        else:
            self._v2p_recur = list(range(h.num_layers))
        self._v2p_no_recur = list(range(h.num_layers))
        self.v2p = self._v2p_no_recur
        self._update_layer_counts(h.num_layers)
        # Use max virtual layer count for skip weights (with recurrence active)
        max_vl = len(self._v2p_recur) if self.recur_layers else h.num_layers
        max_enc = max_vl // 2
        max_dec = max_vl - max_enc
        max_skip = min(max_enc, max_dec)
        self.skip_weights = nn.Parameter(torch.ones(max_skip, h.model_dim, dtype=torch.float32))
        self.skip_gates = nn.Parameter(torch.zeros(max_skip, h.model_dim, dtype=torch.float32)) if h.skip_gates_enabled else None
    ...
    def _update_layer_counts(self, num_physical_layers: int) -> None:
        vl = len(self.v2p)
        self.num_encoder_layers = vl // 2
        self.num_decoder_layers = vl - self.num_encoder_layers
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[pi](x, x0, v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            vi = self.num_encoder_layers + i
            pi = v2p[vi]
            skip_idx = min(i, len(skips) - 1) if skips else -1
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 10, eps: float = 1e-7,
                                 row_normalize: bool = False) -> Tensor:
    a, b, c = (3.4445, -4.7750, 2.0315)
    X = G.bfloat16()
    if row_normalize:
        # MuonEq-R: normalize each row by its L2 norm before NS5
...
def _select_ttt_blocks(h: Hyperparameters) -> set[int]:
    layers_config = h.spectral_ttt_layers
    n = h.num_layers
    if layers_config == 'last2':
        return set(range(n - 2, n))
    elif layers_config == 'last3':
        return set(range(n - 3, n))
    elif layers_config == 'all':
        return set(range(n))
    return set(int(x) for x in layers_config.split(',') if x.strip())


def spectral_ttt_eval(
    h: Hyperparameters,
    device: torch.device,
    val_data: ValidationData,
    model: GPT,
) -> tuple[float, float]:
    """Score-first Spectral TTT: adapt singular values of weight matrices at eval time.

    For each chunk of val data:
      1. Score the chunk using current model weights (no gradients)
      2. Compute SVD-projected gradients on already-scored tokens
      3. Update singular values only (U and V frozen)

    This is legal because every token is scored BEFORE any gradient from it is used.
    """
    log("spectral_ttt:starting")
    seq_len = h.eval_seq_len
    stride = h.eval_stride
    chunk_tokens = h.spectral_ttt_chunk_tokens
    total_tokens = val_data.val_tokens.numel() - 1
    context_size = seq_len - stride

    target_blocks = _select_ttt_blocks(h)

    # Collect weight matrices to adapt and compute their SVDs
    # Store: (param_ref, U, Vh, sigma_delta) where sigma_delta is the learnable offset
    adaptations: list[dict] = []
    for bi in target_blocks:
        block = model.blocks[bi]
        for sub_name, sub_module in [('attn', block.attn), ('mlp', block.mlp)]:
            for pname, param in sub_module.named_parameters():
                if param.ndim == 2 and param.numel() > 1024:
                    W = param.detach().float()
                    U, S, Vh = torch.linalg.svd(W, full_matrices=False)
                    # sigma_delta: learnable offset to singular values (init at 0)
                    sigma_delta = torch.zeros_like(S, requires_grad=True, device=device)
                    adaptations.append({
                        'param': param,
                        'U': U.to(param.dtype),
                        'S_base': S.to(param.dtype),
                        'Vh': Vh.to(param.dtype),
                        'sigma_delta': sigma_delta,
                        'original': param.detach().clone(),
                        'name': f'block{bi}.{sub_name}.{pname}',
                    })

    # Also adapt the embedding/head if tied
    if model.tie_embeddings:
        param = model.tok_emb.weight
        W = param.detach().float()
        U, S, Vh = torch.linalg.svd(W, full_matrices=False)
        sigma_delta = torch.zeros_like(S, requires_grad=True, device=device)
        adaptations.append({
    ...
    total_sigma_params = sum(a['sigma_delta'].numel() for a in adaptations)
    log(f"spectral_ttt:{len(adaptations)} matrices, {total_sigma_params} sigma params")

    optimizer = torch.optim.AdamW(
        [a['sigma_delta'] for a in adaptations],
        lr=h.spectral_ttt_lr,
        weight_decay=h.spectral_ttt_wd,
    )

    def apply_adaptations():
        """Write W = U @ diag(S_base + sigma_delta) @ Vh into model weights."""
        with torch.no_grad():
            for a in adaptations:
                new_S = a['S_base'] + a['sigma_delta'].to(a['S_base'].dtype)
                W_new = (a['U'] * new_S.unsqueeze(0)) @ a['Vh']
                a['param'].copy_(W_new)

    def restore_weights():
        with torch.no_grad():
            for a in adaptations:
                a['param'].copy_(a['original'])

    # Split val data into chunks
    chunk_starts = list(range(0, total_tokens, chunk_tokens))
    n_chunks = len(chunk_starts)
    total_opt_steps = max(1, (n_chunks - 1) * h.spectral_ttt_epochs)
    opt_step = 0

    total_loss_sum = torch.tensor(0.0, dtype=torch.float64, device=device)
    total_token_count = torch.tensor(0.0, dtype=torch.float64, device=device)
    total_byte_count = torch.tensor(0.0, dtype=torch.float64, device=device)

    for ci, cs in enumerate(chunk_starts):
        ce = min(cs + chunk_tokens, total_tokens)

        # Phase 1: SCORE this chunk with current weights
        apply_adaptations()
        model.eval()
        window_starts = [ws for ws in range(cs, ce, stride)
        ...
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            for ws in window_starts:
                ...
                if s < wlen:
                    ...
                    total_token_count += float(wlen - s)
                    tgt = y[0, s:wlen]
                    prev = x[0, s:wlen]
        ...
        # Phase 2: TRAIN on this chunk (skip last chunk — never train on unscored data)
        if ci < n_chunks - 1:
            train_chunk = val_data.val_tokens[cs:ce + 1].to(device)
            chunk_len = ce - cs

            for epoch in range(h.spectral_ttt_epochs):
                # Cosine LR decay across all opt steps
                cos_frac = opt_step / total_opt_steps
                lr_now = h.spectral_ttt_lr * 0.5 * (1.0 + math.cos(math.pi * cos_frac))
                for pg in optimizer.param_groups:
                    pg['lr'] = lr_now

                # Apply current sigma to weights, then forward + backward
                apply_adaptations()

                # Use full chunk as one sequence (or split into windows)
                n_windows = max(1, chunk_len // seq_len)
                for wi in range(n_windows):
                    w_start = wi * seq_len
                    if w_start >= chunk_len:
                        break
                    w_end = min(w_start + seq_len, chunk_len)
                    x = train_chunk[w_start:w_end].unsqueeze(0)
                    y = train_chunk[w_start + 1:w_end + 1].unsqueeze(0)
                    if x.shape[1] == 0 or x.shape[1] != y.shape[1]:
                        continue
                    ...
                    # Backward through model weights
                    loss.backward()

                    # Project weight gradients onto spectral basis:
                    # dL/d(sigma_delta)_i = (U^T @ dL/dW @ Vh^T)_{ii}
                    optimizer.zero_grad()
                    for a in adaptations:
                        if a['param'].grad is not None:
                            gW = a['param'].grad.float()
                            # Efficient: diag(U^T @ gW @ Vh^T) = sum(U * (gW @ Vh^T), dim=0)
                            grad_sigma = torch.sum(
                                a['U'].float() * (gW @ a['Vh'].float().T), dim=0
                            )
                            a['sigma_delta'].grad = grad_sigma.to(a['sigma_delta'].dtype)
                            a['param'].grad = None  # Clear model weight grad

                    optimizer.step()

                opt_step += 1

            if h.is_main_process and (ci % 10 == 0 or ci == n_chunks - 2):
                log(f"spectral_ttt:chunk {ci+1}/{n_chunks}")
    ...
    val_loss, val_bpb = _loss_bpb(total_loss_sum, total_token_count, total_byte_count)
    log(f"spectral_ttt:final val_loss={val_loss:.6f} val_bpb={val_bpb:.6f}")
...
def run_evals(
    ...
    if h.spectral_ttt_enabled:
        timed_eval("spectral_ttt", spectral_ttt_eval, h, device, val_data, eval_model)

# -----------------------------
# Training
# -----------------------------

def _wrap_model(base_model, h):
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
    if h.distributed:
        return DDP(compiled_model, device_ids=[h.local_rank], broadcast_buffers=False), compiled_model
    return compiled_model, compiled_model

def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData) -> None:
    # Set up model
    base_model = GPT(h).to(device).bfloat16()
    restore_fp32_params(base_model)
    model_ref = [None, None]  # [wrapped_model, compiled_model] — mutable for closure
    model_ref[0], model_ref[1] = _wrap_model(base_model, h)
    ...
    if h.warmup_steps > 0:
        ...
        optimizers.zero_grad_all()
        if h.distributed:
            model_ref[0].require_backward_grad_sync = True
        train_loader = DistributedTokenLoader(
    ...
    while True:
        ...
        if should_validate:
            ...
            val_loss, val_bpb = eval_val(h, device, val_data, model_ref[0])
        ...
        # Activate depth recurrence after recur_start_step
        if (base_model.recur_layers and not base_model._recurrence_active
                and step >= h.recur_start_step):
            base_model.set_recurrence_active(True)
            # Recompile after architecture change
            model_ref[0], model_ref[1] = _wrap_model(base_model, h)
            log(f"recurrence:activated step={step} layers={base_model.recur_layers} "
                f"virtual_layers={len(base_model.v2p)}")
...
def train_and_eval(h: Hyperparameters, device: torch.device) -> None:
    ...
    # Ensure recurrence is active for eval
    if base_model.recur_layers and not base_model._recurrence_active:
        base_model.set_recurrence_active(True)
