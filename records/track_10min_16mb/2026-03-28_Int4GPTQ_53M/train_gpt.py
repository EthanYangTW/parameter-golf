"""V27c: 21-level quantization (clip_range=10) + 42M model (dim=576, MHA 8/8, BigramHash 8192)."""
...
class Hyperparameters:
    ...
    quant_clip_range = int(os.environ.get("QUANT_CLIP_RANGE", 10))  # 10 = 21 levels, 7 = 15 (int4), 15 = 31 (int5)
...
class CastedLinear(nn.Linear):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._clip_range = int(os.environ.get("QUANT_CLIP_RANGE", 10))  # 10 = 21 levels
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 10) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
...
def _find_best_row_scales(W: Tensor, clip_range: int = 10) -> Tensor:
...
def gptq_quantize_weight(W: Tensor, H: Tensor, clip_range: int = 10,
...
def mixed_quantize_int6_gptq(state_dict: dict[str, Tensor], int6_cats: set[str],
                              hessians: dict[str, Tensor],
                              clip_range: int = 10) -> tuple[dict, dict]:
    """GPTQ quantization with configurable precision."""
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim == 2:
            ...
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    print(f"gptq_quantize: {gptq_count} GPTQ layers, {naive_count} naive layers (cr={cr})", flush=True)
    return result, meta
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6_gptq(sd_cpu, {"mlp", "attn"}, gptq_hessians, clip_range=args.quant_clip_range)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model quant+{_COMPRESSOR}: {quant_file_bytes} bytes")
        log0(f"Total submission size quant+{_COMPRESSOR}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    with open("final_model.int6.ptz", "rb") as f:
        quant_blob_disk = f.read()
    ...
    torch.cuda.synchronize()
    log0(
        f"final_quant_ttt val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
        f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_ttt):.0f}ms"
    )
    log0(f"final_quant_ttt_exact val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f}")
    if distributed:
        dist.destroy_process_group()
